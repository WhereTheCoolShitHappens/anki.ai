//
// OrderDetailsExtendedsMasterViewController.swift
// AnkiAi-iOS
//
...
class OrderDetailsExtendedsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [OrderDetailsExtended] = [OrderDetailsExtended]()
    private let logger = Logger.shared(named: "OrderDetailsExtendedMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let orderdetailsextended = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Discount", value: "\(orderdetailsextended.discount!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchOrderDetailsExtendeds(matching: query) { orderDetailsExtendeds, error in
            guard let orderDetailsExtendeds = orderDetailsExtendeds else {
                completionHandler(error!)
                return
            }
            self.entities = orderDetailsExtendeds
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! OrderDetailsExtendedsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].discount != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].discount!)" : "")"
...
extension OrderDetailsExtendedsMasterViewController: EntitySetUpdaterDelegate {
