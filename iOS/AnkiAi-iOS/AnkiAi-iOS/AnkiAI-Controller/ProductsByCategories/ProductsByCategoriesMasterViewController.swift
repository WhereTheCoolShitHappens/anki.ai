//
// ProductsByCategoriesMasterViewController.swift
// AnkiAi-iOS
//
...
class ProductsByCategoriesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [ProductsByCategory] = [ProductsByCategory]()
    private let logger = Logger.shared(named: "ProductsByCategoryMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let productsbycategory = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CategoryName", value: "\(productsbycategory.categoryName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchProductsByCategories(matching: query) { productsByCategories, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ProductsByCategoriesDetailViewController
...
extension ProductsByCategoriesMasterViewController: EntitySetUpdaterDelegate {
