//
// OrdersMasterViewController.swift
// AnkiAi-iOS
//
...
class OrdersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Order] = [Order]()
    private let logger = Logger.shared(named: "OrderMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let order = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(order.orderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchOrders(matching: query) { orders, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! OrdersDetailViewController
...
extension OrdersMasterViewController: EntitySetUpdaterDelegate {
