//
// ShippersMasterViewController.swift
// AnkiAi-iOS
//
...
class ShippersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Shipper] = [Shipper]()
    private let logger = Logger.shared(named: "ShipperMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let shipper = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ShipperID", value: "\(shipper.shipperID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchShippers(matching: query) { shippers, error in
            guard let shippers = shippers else {
                completionHandler(error!)
                return
            }
            self.entities = shippers
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ShippersDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].shipperID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].shipperID!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! ShippersDetailViewController
...
extension ShippersMasterViewController: EntitySetUpdaterDelegate {
