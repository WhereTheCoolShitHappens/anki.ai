//
// ProductsAboveAveragePricesMasterViewController.swift
// AnkiAi-iOS
//
...
class ProductsAboveAveragePricesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [ProductsAboveAveragePrice] = [ProductsAboveAveragePrice]()
    private let logger = Logger.shared(named: "ProductsAboveAveragePriceMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let productsaboveaverageprice = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductName", value: "\(productsaboveaverageprice.productName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchProductsAboveAveragePrices(matching: query) { productsAboveAveragePrices, error in
            guard let productsAboveAveragePrices = productsAboveAveragePrices else {
                completionHandler(error!)
                return
            }
            self.entities = productsAboveAveragePrices
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ProductsAboveAveragePricesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].productName ?? ""
            ...
            let detailViewController = dest.viewControllers[0] as! ProductsAboveAveragePricesDetailViewController
...
extension ProductsAboveAveragePricesMasterViewController: EntitySetUpdaterDelegate {
