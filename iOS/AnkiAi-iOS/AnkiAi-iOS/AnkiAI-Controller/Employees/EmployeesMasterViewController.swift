//
// EmployeesMasterViewController.swift
// AnkiAi-iOS
//
...
class EmployeesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Employee] = [Employee]()
    private let logger = Logger.shared(named: "EmployeeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let employee = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "EmployeeID", value: "\(employee.employeeID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchEmployees(matching: query) { employees, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! EmployeesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].employeeID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].employeeID!)" : "")"
...
extension EmployeesMasterViewController: EntitySetUpdaterDelegate {
