//
// SummaryOfSalesByYearsMasterViewController.swift
// AnkiAi-iOS
//
...
class SummaryOfSalesByYearsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SummaryOfSalesByYear] = [SummaryOfSalesByYear]()
    private let logger = Logger.shared(named: "SummaryOfSalesByYearMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let summaryofsalesbyyear = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(summaryofsalesbyyear.orderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchSummaryOfSalesByYears(matching: query) { summaryOfSalesByYears, error in
            guard let summaryOfSalesByYears = summaryOfSalesByYears else {
                completionHandler(error!)
                return
            }
            self.entities = summaryOfSalesByYears
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SummaryOfSalesByYearsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].orderID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].orderID!)" : "")"
...
extension SummaryOfSalesByYearsMasterViewController: EntitySetUpdaterDelegate {
