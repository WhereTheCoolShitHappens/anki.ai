//
// RegionsMasterViewController.swift
// AnkiAi-iOS
//
...
class RegionsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Region] = [Region]()
    private let logger = Logger.shared(named: "RegionMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let region = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "RegionID", value: "\(region.regionID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchRegions(matching: query) { regions, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! RegionsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].regionID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].regionID!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! RegionsDetailViewController
...
extension RegionsMasterViewController: EntitySetUpdaterDelegate {
