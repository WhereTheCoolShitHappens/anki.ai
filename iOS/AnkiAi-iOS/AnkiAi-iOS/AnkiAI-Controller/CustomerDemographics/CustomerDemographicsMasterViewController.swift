//
// CustomerDemographicsMasterViewController.swift
// AnkiAi-iOS
//
...
class CustomerDemographicsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [CustomerDemographic] = [CustomerDemographic]()
    private let logger = Logger.shared(named: "CustomerDemographicMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let customerdemographic = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CustomerTypeID", value: "\(customerdemographic.customerTypeID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchCustomerDemographics(matching: query) { customerDemographics, error in
            guard let customerDemographics = customerDemographics else {
                completionHandler(error!)
                return
            }
            self.entities = customerDemographics
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! CustomerDemographicsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].customerTypeID ?? ""
...
extension CustomerDemographicsMasterViewController: EntitySetUpdaterDelegate {
