//
// OrderDetailsMasterViewController.swift
// AnkiAi-iOS
//
...
class OrderDetailsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [OrderDetail] = [OrderDetail]()
    private let logger = Logger.shared(named: "OrderDetailMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let orderdetail = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(orderdetail.orderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchOrderDetails(matching: query) { orderDetails, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! OrderDetailsDetailViewController
...
extension OrderDetailsMasterViewController: EntitySetUpdaterDelegate {
