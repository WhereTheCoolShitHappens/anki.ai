//
// OrderSubtotalsMasterViewController.swift
// AnkiAi-iOS
//
...
class OrderSubtotalsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [OrderSubtotal] = [OrderSubtotal]()
    private let logger = Logger.shared(named: "OrderSubtotalMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let ordersubtotal = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(ordersubtotal.orderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchOrderSubtotals(matching: query) { orderSubtotals, error in
            guard let orderSubtotals = orderSubtotals else {
                completionHandler(error!)
                return
            }
            self.entities = orderSubtotals
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! OrderSubtotalsDetailViewController
...
extension OrderSubtotalsMasterViewController: EntitySetUpdaterDelegate {
