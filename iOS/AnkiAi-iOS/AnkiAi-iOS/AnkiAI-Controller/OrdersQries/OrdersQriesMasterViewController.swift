//
// OrdersQriesMasterViewController.swift
// AnkiAi-iOS
//
...
class OrdersQriesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [OrdersQry] = [OrdersQry]()
    private let logger = Logger.shared(named: "OrdersQryMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let ordersqry = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CompanyName", value: "\(ordersqry.companyName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchOrdersQries(matching: query) { ordersQries, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! OrdersQriesDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! OrdersQriesDetailViewController
...
extension OrdersQriesMasterViewController: EntitySetUpdaterDelegate {
