//
// SuppliersMasterViewController.swift
// AnkiAi-iOS
//
...
class SuppliersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private let logger = Logger.shared(named: "SupplierMasterViewConterollerLogger")
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchSuppliers(matching: query) { suppliers, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SuppliersDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].supplierID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].supplierID!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! SuppliersDetailViewController
...
extension SuppliersMasterViewController: EntitySetUpdaterDelegate {
