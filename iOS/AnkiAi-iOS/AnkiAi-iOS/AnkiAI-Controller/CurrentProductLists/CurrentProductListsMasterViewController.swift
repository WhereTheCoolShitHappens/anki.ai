//
// CurrentProductListsMasterViewController.swift
// AnkiAi-iOS
//
...
class CurrentProductListsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [CurrentProductList] = [CurrentProductList]()
    private let logger = Logger.shared(named: "CurrentProductListMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let currentproductlist = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ProductID", value: "\(currentproductlist.productID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchCurrentProductLists(matching: query) { currentProductLists, error in
            guard let currentProductLists = currentProductLists else {
                completionHandler(error!)
                return
            }
            self.entities = currentProductLists
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! CurrentProductListsDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].productID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].productID!)" : "")"
...
extension CurrentProductListsMasterViewController: EntitySetUpdaterDelegate {
