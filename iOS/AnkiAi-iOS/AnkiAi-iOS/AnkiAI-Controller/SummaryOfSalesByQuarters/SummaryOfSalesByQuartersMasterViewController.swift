//
// SummaryOfSalesByQuartersMasterViewController.swift
// AnkiAi-iOS
//
...
class SummaryOfSalesByQuartersMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SummaryOfSalesByQuarter] = [SummaryOfSalesByQuarter]()
    private let logger = Logger.shared(named: "SummaryOfSalesByQuarterMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let summaryofsalesbyquarter = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "OrderID", value: "\(summaryofsalesbyquarter.orderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchSummaryOfSalesByQuarters(matching: query) { summaryOfSalesByQuarters, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SummaryOfSalesByQuartersDetailViewController
...
extension SummaryOfSalesByQuartersMasterViewController: EntitySetUpdaterDelegate {
