//
// InvoicesMasterViewController.swift
// AnkiAi-iOS
//
...
class InvoicesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Invoice] = [Invoice]()
    private let logger = Logger.shared(named: "InvoiceMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let invoice = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CustomerName", value: "\(invoice.customerName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchInvoices(matching: query) { invoices, error in
            guard let invoices = invoices else {
                completionHandler(error!)
                return
            }
            self.entities = invoices
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! InvoicesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].customerName ?? ""
...
extension InvoicesMasterViewController: EntitySetUpdaterDelegate {
