//
// SalesByCategoriesMasterViewController.swift
// AnkiAi-iOS
//
...
class SalesByCategoriesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SalesByCategory] = [SalesByCategory]()
    private let logger = Logger.shared(named: "SalesByCategoryMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesbycategory = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CategoryID", value: "\(salesbycategory.categoryID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchSalesByCategories(matching: query) { salesByCategories, error in
            guard let salesByCategories = salesByCategories else {
                completionHandler(error!)
                return
            }
            self.entities = salesByCategories
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SalesByCategoriesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].categoryID != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].categoryID!)" : "")"
            ...
            let detailViewController = dest.viewControllers[0] as! SalesByCategoriesDetailViewController
...
extension SalesByCategoriesMasterViewController: EntitySetUpdaterDelegate {
