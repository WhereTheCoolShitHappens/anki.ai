//
// SalesTotalsByAmountsMasterViewController.swift
// AnkiAi-iOS
//
...
class SalesTotalsByAmountsMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SalesTotalsByAmount] = [SalesTotalsByAmount]()
    private let logger = Logger.shared(named: "SalesTotalsByAmountMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salestotalsbyamount = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CompanyName", value: "\(salestotalsbyamount.companyName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchSalesTotalsByAmounts(matching: query) { salesTotalsByAmounts, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SalesTotalsByAmountsDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! SalesTotalsByAmountsDetailViewController
...
extension SalesTotalsByAmountsMasterViewController: EntitySetUpdaterDelegate {
