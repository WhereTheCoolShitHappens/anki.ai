//
// CustomerAndSuppliersByCitiesMasterViewController.swift
// AnkiAi-iOS
//
...
class CustomerAndSuppliersByCitiesMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [CustomerAndSuppliersByCity] = [CustomerAndSuppliersByCity]()
    private let logger = Logger.shared(named: "CustomerAndSuppliersByCityMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let customerandsuppliersbycity = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "CompanyName", value: "\(customerandsuppliersbycity.companyName!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.northwindEntities.fetchCustomerAndSuppliersByCities(matching: query) { customerAndSuppliersByCities, error in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! CustomerAndSuppliersByCitiesDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].companyName ?? ""
            ...
            let detailViewController = dest.viewControllers[0] as! CustomerAndSuppliersByCitiesDetailViewController
...
extension CustomerAndSuppliersByCitiesMasterViewController: EntitySetUpdaterDelegate {
